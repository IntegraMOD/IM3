...
// <![CDATA[
var myurl = "<?php echo $this->_rootref['S_AJAX_COLLAPSE_ACTION'] ?? ''; ?>";
//]]>
</script>
<script src="<?php echo $this->_rootref['ROOT_PATH'] ?? ''; ?>assets/js/ccat.js"></script>
<?php $_forumrow_count = (isset($this->_tpldata['forumrow']) && is_array($this->_tpldata['forumrow'])) ? count($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; if (( $_forumrow_val['S_IS_CAT'] && ! $_forumrow_val['S_FIRST_ROW'] ) || $_forumrow_val['S_NO_CAT']) {  ?>
			...
			<ul class="topiclist">
				<li class="header">
					<dl class="icon">
						...
						<dd class="lastposthdr"><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?></dd>
						<dd class="cchdr"><span><input type="button" class="<?php echo $_forumrow_val['COLLAPSE_CLASS']; ?>" value=" " onclick="flipf(this,'<?php echo $_forumrow_val['FORUM_ID']; ?>');" /></span></dd>
					</dl>
				</li>
			</ul>
			<ul class="topiclist forums catpanel" id="flist<?php echo $_forumrow_val['FORUM_ID']; ?>" <?php echo $_forumrow_val['S_COLLAPSE']; ?>>
	<?php } if (! $_forumrow_val['S_IS_CAT']) {  ?>
		...
		<li class="row panel">
			<dl class="icon" style="background-image: url(<?php echo $_forumrow_val['FORUM_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat;">
				...
				<?php if (($this->_rootref['S_ENABLE_FEEDS'] ?? null) && $_forumrow_val['S_FEED_ENABLED']) {  ?><a class="feed-icon-forum" title="<?php echo ((isset($this->_rootref['L_FEED'])) ? $this->_rootref['L_FEED'] : ((isset($user->lang['FEED'])) ? $user->lang['FEED'] : '{ FEED }')); ?> - <?php echo $_forumrow_val['FORUM_NAME']; ?>" href="<?php echo $this->_rootref['U_FEED'] ?? ''; ?>?f=<?php echo $_forumrow_val['FORUM_ID']; ?>"><img src="<?php echo $this->_rootref['T_THEME_PATH'] ?? ''; ?>/images/feed.gif" alt="<?php echo ((isset($this->_rootref['L_FEED'])) ? $this->_rootref['L_FEED'] : ((isset($user->lang['FEED'])) ? $user->lang['FEED'] : '{ FEED }')); ?> - <?php echo $_forumrow_val['FORUM_NAME']; ?>" /></a><?php } if ($_forumrow_val['FORUM_IMAGE']) {  ?><span class="forum-image"><?php echo $_forumrow_val['FORUM_IMAGE']; ?></span><?php } ?>
				...
				<?php } else if (! $_forumrow_val['S_IS_LINK']) {  ?>
					...
					<dd class="lastpost"><span>
						<?php if ($_forumrow_val['U_UNAPPROVED_TOPICS']) {  ?><a href="<?php echo $_forumrow_val['U_UNAPPROVED_TOPICS']; ?>"><?php echo $this->_rootref['UNAPPROVED_IMG'] ?? ''; ?></a><?php } if ($_forumrow_val['LAST_POST_TIME']) {  ?><dfn><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?></dfn> <?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?> <?php echo $_forumrow_val['LAST_POSTER_FULL']; ?>

						<?php if (! ($this->_rootref['S_IS_BOT'] ?? null)) {  ?><a href="<?php echo $_forumrow_val['U_LAST_POST']; ?>"><?php echo $this->_rootref['LAST_POST_IMG'] ?? ''; ?></a> <?php } ?><br /><?php echo $_forumrow_val['LAST_POST_TIME']; } else { echo ((isset($this->_rootref['L_NO_POSTS'])) ? $this->_rootref['L_NO_POSTS'] : ((isset($user->lang['NO_POSTS'])) ? $user->lang['NO_POSTS'] : '{ NO_POSTS }')); ?><br />&nbsp;<?php } ?></span>
					</dd>
				<?php } else { ?>
