...
* @version $Id: js.php 260 2008-02-17 08:46:40Z paul $
		...
		hin2.onreadystatechange = function()
		{
			try
			{
				...
				if (hin2.readyState == 4)
				{
					...
					if (xml.getElementsByTagName('time').length <= 0 || xml.getElementsByTagName('time')[0].childNodes <= 0)
					{
						throw err_msg(lang['SERVER_ERR']);
						return;
					}
									...
									if (hin.readyState == 4)
									{
										...
										else
										{
											...
											for (var i = 0; i < tmp.length ; i++)
											{
															...
															hdelete.onreadystatechange = function()
															{
																try
																{
																	...
																	if (hdelete.readyState == 4)
																	{
																		...
																		{
																			message('<?php echo addslashes($user->lang['MSG_DEL_DONE']); ?>');
																		}
																		setTimeout("document.getElementById('msg_txt').innerHTML = ''",3000);
												...
												{
												?>
												if (inh.getElementsByTagName('edit').length >= 1 &&  inh.getElementsByTagName('edit')[0].childNodes.length >= 1 && inh.getElementsByTagName('edit')[0].childNodes[0].nodeValue)
												{
													msg3 = ce('span');
													msg3.id = 'text' + i;
													...
													input.type = 'button';
													input.id = 'submit' + i;
													input.value = '<?php echo addslashes($user->lang['EDIT']); ?>';
													input.i = i;
													...
													input.onclick = function()
													{
														...
														document.getElementById('text' + i).appendChild(tn('<?php echo addslashes($user->lang['SENDING_EDIT']); ?>'));

														var hedit = http();
														
														if (hedit.readyState == 4 || hedit.readyState == 0)
														{
															hedit.open('POST','<?php echo append_sid("{$phpbb_root_path}ajax.$phpEx",  'm=edit', false)?>&last=' + last + '&rand='+Math.floor(Math.random() * 1000000),true);
															hedit.i = i;
															hedit.onreadystatechange = function()
															{
																try
																{
																	...
																	if ($auth->acl_get('u_as_post'))
																	{
																		echo 'document.getElementById("post_message").style.display = "block";';
																	}
																	?>
																	...
																	{
																		message('<?php echo addslashes($user->lang['EDIT_DONE']);?>');
																	}
																}
																catch (e)
																{
													...
													var input = ce('input');
													input.type = 'button';
													input.value = '<?php echo addslashes($user->lang['CANCEL']); ?>';
													input.i = i;
													...
													edit_button.onclick = function()
													{
														if (one_open)
														{
															alert('<?php echo addslashes($user->lang['ONLY_ONE_OPEN']); ?>');
															return;
														}
														one_open = true;
														...
														if ($auth->acl_get('u_as_post'))
														{
															echo 'document.getElementById("post_message").style.display = "none";';
														}
														?>
