	...
	case 'view':	// Viewing events of a given day.
		...
		if (checkdate($month, $day, $year))
		{
			$template->assign_vars(array(
				...
				'FB_IMG'	=> $user->img('icon_contact_fb', 'FB'),
				'IG_IMG'	=> $user->img('icon_contact_ig', 'IG'),
				'PT_IMG'	=> $user->img('icon_contact_pt', 'PT'),
				'TWR_IMG'	=> $user->img('icon_contact_twr', 'TWR'),
				'SKP_IMG'	=> $user->img('icon_contact_skp', 'SKP'),
				'TG_IMG'	=> $user->img('icon_contact_tg', 'TG'),
				'LI_IMG'	=> $user->img('icon_contact_li', 'LI'),
				'TT_IMG'	=> $user->img('icon_contact_tt', 'TT'),
				'DC_IMG'	=> $user->img('icon_contact_dc', 'DC'),
      		...
      		if (isset($events_array)) 
			{
				foreach ($events_array as $row)
				{
					...
					$template->assign_block_vars('event_row', array(
						...
						'U_FB'				=> ($row['user_fb'])     ? 'https://www.facebook.com/' . urlencode($row['user_fb']) : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=fb&u=' . $row['user_id']),
						'U_IG'				=> ($row['user_ig'])     ? 'https://www.instagram.com/' . urlencode($row['user_ig']) : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=ig&u=' . $row['user_id']),
						'U_PT'				=> ($row['user_pt'])     ? 'https://www.pinterest.com/' . urlencode($row['user_pt']) : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=pt&u=' . $row['user_id']),
						'U_TWR'				=> ($row['user_twr'])    ? 'https://twitter.com/' . urlencode($row['user_twr']) : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=twr&u=' . $row['user_id']),
						'U_SKP'				=> ($row['user_skp'])    ? 'skype:' . urlencode($row['user_skp']) . '?chat' : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=skp&u=' . $row['user_id']),
						'U_TG'				=> ($row['user_tg'])     ? 'https://t.me/' . urlencode($row['user_tg']) : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=tg&u=' . $row['user_id']),
						'U_LI'				=> ($row['user_li'])     ? 'https://www.linkedin.com/in/' . urlencode($row['user_li']) : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=li&u=' . $row['user_id']),
						'U_TT'				=> ($row['user_tt'])     ? 'https://www.tiktok.com/@' . urlencode($row['user_tt']) : append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=tt&u=' . $row['user_id']),
						'U_DC'				=> ($row['user_dc'])     ? append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=contact&action=dc&u=' . $row['user_id']) : '',
