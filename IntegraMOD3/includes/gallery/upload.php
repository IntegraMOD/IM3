 ...
 * @version   GIT: <2>
 * @copyright (c) 2011 nickvergessen http://www.flying-bits.org
...
//   private ?\phpbb_gallery_image_file $tools = null;
    ...
    */
    ...
    private bool $exif_status = false;
    private bool $exif_data = false;
    ...
    */
    public function upload_zip()
    {
        ...
        global $user;
        $tmp_dir = phpbb_gallery_url::path('import') . 'tmp_' . md5((string) unique_id()) . '/';

        $this->zip_file->clean_filename('unique_ext'/*, $user->data['user_id'] . '_'*/);
        $this->zip_file->move_file(substr((string) phpbb_gallery_url::path('import_noroot'), 0, -1), false, false, CHMOD_ALL);
    ...
    */
    public function read_zip_folder($current_dir)
    {
        ...
        while ($file = readdir($handle)) {
            ...
            } elseif (in_array(utf8_substr(strtolower($file), utf8_strrpos($file, '.') + 1), self::get_allowed_types(false, true))) {
                if (!$this->file_limit || ($this->uploaded_files < $this->file_limit)) {
    ...
    */
    public function prepare_file()
    {
        ...
        $this->file->move_file(substr((string) phpbb_gallery_url::path('upload_noroot'), 0, -1), false, false, CHMOD_ALL);
    ...
    */
    public function file_to_database()
    {
        ...
        $image_name = str_replace("_", " ", (string) utf8_substr($this->file->uploadname, 0, utf8_strrpos($this->file->uploadname, '.')));
    ...
    */
    public static function prune_orphan($time = 0)
    {
        global $db;
    ...
    public function use_same_name($use_same_name)
    {
        if ($use_same_name) {
            ...
            for ($i = 0; $i < sizeof($this->file_names); $i++) {
                $this->file_names[$i] = str_replace('{NUM}', ($this->image_num + $i), (string) $image_name);
                $this->file_descriptions[$i] = str_replace('{NUM}', ($this->image_num + $i), (string) $image_desc);
            }
        }
    }
    ...
    public function get_images($uploaded_ids)
    {
        ...
        foreach ($uploaded_ids as $row => $check) {
            if (!str_contains((string) $check, '$')) {
                continue;
            }
            [$image_id, $filename] = explode('$', (string) $check);
        ...
        while ($row = $db->sql_fetchrow($result)) {
            if ($filenames[$row['image_id']] == substr((string) $row['image_filename'], 0, 8)) {
    ...
    */
    public static function get_allowed_types($get_types = false, $ignore_zip = false)
    {
        global $user;
    ...
    */
    public function generate_hidden_fields()
    {
        ...
        foreach ($this->images as $image_id) {
            $checks[] = $image_id . '$' . substr((string) $this->image_data[$image_id]['image_filename'], 0, 8);
        }
        return $checks;
    }
}