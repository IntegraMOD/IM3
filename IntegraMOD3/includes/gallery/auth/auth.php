...
class phpbb_gallery_auth
{
    ...
    private static array $_permission_m = array('m_comments', 'm_delete', 'm_edit', 'm_move', 'm_report', 'm_status');
    private static array $_permission_misc = array('a_list', 'i_count', 'i_unlimited', 'a_count', 'a_unlimited', 'a_restrict');
    private static array $_permissions = array();
    private static $_permissions_flipped = array();

    private array $_auth_data = array();
    private array $_auth_data_never = array();

    private array $acl_cache = array();
    ...
    */
    public function __construct($user_id, $album_id = false)
    {
        ...
        if (($user_id == $user->data['user_id']) && !empty($cached_permissions)) {
            ...
            return;
        } elseif ($user_id != $user->data['user_id']) {
            global $db;
    ...
    */
    private function query_auth_data($user_id)
    {
        ...
        static::set_user_permissions($user_id, $this->_auth_data);
    }
    ...
    private function restrict_pegas($user_id)
    {
        ...
        foreach ($albums as $album) {
            if (!$album['album_auth_access'] || ($album['album_user_id'] == self::PUBLIC_ALBUM)) {# || ($album['album_user_id'] == $user_id))
                ...
                $this->_auth_data[$album['album_id']] = new phpbb_gallery_auth_set();
                continue;
            } elseif ($album['album_auth_access'] == self::ACCESS_NOT_FOES) {
                if ($zebra == null) {
            ...
            } elseif ($album['album_auth_access'] == self::ACCESS_FRIENDS) {
                if ($zebra == null) {
                    $zebra = self::get_user_zebra($user_id);
                }
    ...
    */
    public static function get_user_zebra($user_id)
    {
        global $db;
    ...
    */
    public static function get_usergroups($user_id)
    {
        global $config, $db;
    ...
    */
    public static function set_user_permissions($user_ids, $permissions = false)
    {
        global $db, $user, $sql_set;
    ...
    public function acl_album_ids($acl, $return = 'array', $display_in_rrc = false, $display_pegas = true)
    {
        ...
        foreach ($albums as $album) {
            if ($album['album_user_id'] == $user->data['user_id']) {
                $a_id = self::OWN_ALBUM;
            } elseif ($album['album_user_id'] > self::PUBLIC_ALBUM) {
