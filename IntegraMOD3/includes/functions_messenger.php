...
class queue
{
	...
	*/
	function process($process_all=false)
	{
		...
		if ((!$process_all) && (!$have_cache_file || $config['last_queue_run'] > time() - $config['queue_interval']))
		{
			if (!$have_cache_file)
			{
		...
		foreach ($this->queue_data as $object => $data_ary)
		{
			...
			$num_items = (!$package_size || count($data_ary['data']) < $package_size || $process_all) ? count($data_ary['data']) : $package_size;
