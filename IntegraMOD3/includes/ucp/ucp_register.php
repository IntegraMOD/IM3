	...
	function main($id, $mode)
	{
		...
		if ($submit)
		{
			...
			if (!sizeof($error))
			{
				...
				// Begin Digest Mod
				// Check to see if the user wants a digest subscription
				$subscribe = (request_var('digest', '') == '1') ? true : false;
				// End Digest Mod
				...
				// Register user...
				$user_id = user_add($user_row, $cp_data, $subscribe);
				// START Anti-Spam ACP
		...
		$template->assign_vars(array(
			...
			//End: Group on Registration and Custom Profile Fields
			// Begin Digest Mod
			'S_DIGEST_REGISTER_CHECKED_YES' => ($config['digests_user_digest_registration']) ? true : false,
			'S_DIGEST_REGISTER_CHECKED_NO' 	=> ($config['digests_user_digest_registration']) ? false : true,
			'S_DIGESTS'			=> (!$config['digests_enable_auto_subscriptions'] && $config['digests_registration_field']) ? true : false,
			// End Digest Mod
		));
