 ...
 * Grab MOD titles.
 *
 * @param array $header - variable holding all relevant tag information
 *
 * @return array|string
 */
 ...
 *
 * @param string|array $title - string or a serialized array containing the MOD name(s)
 ...
 *
 * @param array $header - variable holding all relevant tag information
 * @param string $tagname - tag name to fetch
 * @param int|false $index - Index number to pull.  Not required.
 *
 * @return string $output - Localised contents of the tag in question
 */
function localise_tags($header, $tagname, $index = false)
{
    ...
    if (isset($header[$tagname]) && is_array($header[$tagname])) {
        foreach ($header[$tagname] as $i => $void) {
            ...
            } elseif (match_language('en', $header[$tagname][$i]['attrs']['LANG'])) {
                $output = isset($header[$tagname][$i]['data']) ? htmlspecialchars(trim($header[$tagname][$i]['data'])) : '';
            }
        }
 ...
 * @param int number of subdirectory levels to scan (set to 1 to scan only current)
 * @param bool List sub-directories only instead of files
 * @param int this one is used internally to control recursion level
 *
...
function determine_write_method($preview = false)
{
    ...
    if ((WRITE_DIRECT == $config['write_method'] && is_writable($phpbb_root_path)) || $preview) {
        $write_method = 'direct';
    }
    // FTP Method is now auto-detected
    elseif (WRITE_FTP == $config['write_method']) {
        $write_method = 'ftp';
    }
    // or zip or tarballs
    elseif ($config['compress_method']) {
        $write_method = 'manual';
    } else {
...
function handle_ftp_details($method, $test_ftp_connection, $test_connection)
{
    ...
    $s_hidden_fields = build_hidden_fields(['method' => $method]);
 
    if (!class_exists($method)) {
        trigger_error('Method does not exist.', E_USER_ERROR);
    }
 
    $requested_data = call_user_func([$method, 'data']);
    ...
    $template->assign_vars([
        'S_CONNECTION_SUCCESS' => ($test_ftp_connection && true === $test_connection) ? true : false,
        'S_CONNECTION_FAILED' => ($test_ftp_connection && true !== $test_connection) ? true : false,
        'ERROR_MSG' => ($test_ftp_connection && true !== $test_connection) ? $user->lang[$test_connection] : '',
...
function get_connection_info($preview = false)
{
    ...
    if (WRITE_FTP == $config['write_method']) {
        if (!$preview && isset($_POST['password'])) {
            ...
            $requested_data = call_user_func([$config['ftp_method'], 'data']);
