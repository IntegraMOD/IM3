...
{
	var $profile_types = array(FIELD_INT => 'int', FIELD_STRING => 'string', FIELD_TEXT => 'text', FIELD_BOOL => 'bool', FIELD_DROPDOWN => 'dropdown', FIELD_DATE => 'date', FIELD_MULTI => 'multi');
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if (($cp_result = $this->validate_profile_field($row['field_type'], $check_value, $row)) !== false)
			{
				...
				switch ($cp_result)
				{
					...
					break;
					case 'FIELD_MULTI_TOO_LOW':
						$error = sprintf($user->lang[$cp_result], $row['lang_name'], $row['field_minlen']);
					break;
					
					case 'FIELD_MULTI_TOO_MANY':
						$error = sprintf($user->lang[$cp_result], $row['lang_name'], $row['field_maxlen']);
					break;
	...
	function get_profile_value($ident_ary)
	{
		...
		switch ($this->profile_types[$field_type])
		{
			...
			case 'multi':
				...
				if (!isset($this->options_lang[$field_id][$lang_id]))
				{
					$this->get_option_lang($field_id, $lang_id, FIELD_MULTI, false);
				}
				...
				$values = explode(FIELD_SEPARATOR, $value);
				$value = '';
				
				foreach ($values as $row)
				{
					if (!isset($this->options_lang[$field_id][$lang_id][$row]))
					{
						continue;
					}
					$value .= (empty($value)) ? $this->options_lang[$field_id][$lang_id][$row] : ', ' . $this->options_lang[$field_id][$lang_id][$row];
				}
				return $value;
			break;
	...
	*/
	function get_var($field_validation, &$profile_row, $default_value, $preview)
	{
		...
		switch ($field_validation)
		{
			...
			break;
			case 'multi':
				return explode(FIELD_SEPARATOR, $value);
			break;
		}
	...
	* @access private
	*/
	function generate_multi($profile_row, $preview = false)
	{
		global $user, $template;
				
		$value = $this->get_var('multi', $profile_row, $profile_row['field_default_value'], $preview);

if (!isset($this->options_lang[$profile_row['field_id']]) || !isset($this->options_lang[$profile_row['field_id']][$profile_row['lang_id']]) || !sizeof($this->options_lang[$profile_row['field_id']][$profile_row['lang_id']]))
		{
			$this->get_option_lang($profile_row['field_id'], $profile_row['lang_id'], FIELD_MULTI, $preview);
		}
		...
		foreach ($this->options_lang[$profile_row['field_id']][$profile_row['lang_id']] as $option_id => $option_value)
		{
			$template->assign_block_vars('multi.options', array(
				'OPTION_ID'	=> $option_id,
	...
	*/
	function get_profile_field($profile_row)
	{
		...
		switch ($profile_row['field_type'])
		{
			...
			break;
			case FIELD_MULTI:
				$var = implode(FIELD_SEPARATOR, request_var($var_name, explode(FIELD_SEPARATOR, $profile_row['field_default_value'])));
			break;
...
class custom_profile_admin extends custom_profile
{
	...
	*/
	...
	/**
	* Get multi options for second step in ACP
	*/
	function get_multi_options()
	{
		global $user;

		$options = array(
			0 => array('TITLE' => $user->lang['FIELD_MULTI_MIN_CHARS'],	'FIELD' => '<input type="text" name="field_minlen" size="10" value="' . $this->vars['field_minlen'] . '" />'),
			1 => array('TITLE' => $user->lang['FIELD_MULTI_MAX_CHARS'],	'FIELD' => '<input type="text" name="field_maxlen" size="10" value="' . $this->vars['field_maxlen'] . '" />'),
																																							 		);
