...
class acp_profile
{
	...
	function main($id, $mode)
	{
		...
		$default_values = array(
			...
			FIELD_MULTI	=> array('field_length' => 0, 'field_minlen' => 0, 'field_maxlen' => 5, 'field_validation' => '', 'field_novalue' => 0, 'field_default_value' => 0),
		);
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				if ($field_type == FIELD_BOOL || $field_type == FIELD_DROPDOWN || $field_type == FIELD_MULTI)
				{
...
if (($field_type == FIELD_DROPDOWN || $field_type == FIELD_MULTI) && !sizeof($cp->vars['lang_options']))
					{
				...
				'S_MULTI'		=> ($field_type == FIELD_MULTI) ? true : false,

								'L_LANG_OPTIONS_EXPLAIN'	=> $user->lang[strtoupper($cp->profile_types[$field_type]) . '_ENTRIES_EXPLAIN'],
'LANG_OPTIONS'				=> ($field_type == FIELD_DROPDOWN || $field_type == FIELD_MULTI) ? implode("\n", $cp->vars['lang_options']) : '',
