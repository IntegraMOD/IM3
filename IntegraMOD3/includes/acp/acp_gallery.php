...
class acp_gallery
{
    ...
    public function overview()
    {
        ...
        'S_SELECT_ALBUM'		=> (new phpbb_gallery_album)->get_albumbox(false, 'reset_album_id', false, false, false, phpbb_gallery_album::PUBLIC_ALBUM, phpbb_gallery_album::TYPE_UPLOAD),
    ...
    public function import()
    {
        ...
        if ($import_schema) {
            if (phpbb_gallery_url::_file_exists($import_schema, 'import', '')) {
                ...
                $user_data['username'] = utf8_substr(str_replace("{{$import_schema}}", "'", (string) $user_data['username']), 0, -1);
                $image_name = utf8_substr(str_replace("{{$import_schema}}", "'", (string) $image_name), 0, -1);
            } else {
                global $phpEx;
            ...
            foreach ($images as $image_src) {
                ...
                * Import the images
                */
                $image_src = str_replace("{{$import_schema}}", "'", (string) $image_src);
                $image_src_full = phpbb_gallery_url::path('import') . mb_convert_encoding($image_src, 'ISO-8859-1');
                if (file_exists($image_src_full)) {
                    ...
                    $image_filename = md5((string) unique_id()) . $filetype_ext;
                    ...
                    if (!$error_occured) {
                        ...
                        if ($filename || ($image_name == '')) {
                            $sql_ary['image_name'] = str_replace("_", " ", (string) utf8_substr($image_src, 0, utf8_strrpos($image_src, '.')));
                        } else {
                            $sql_ary['image_name'] = str_replace('{NUM}', $num_offset + $done_images, (string) $image_name);
                        }
    ...
    public function create_import_schema($import_schema, $album_id, $user_row, $start_time, $num_offset, $done_images, $todo_images, $image_name, $filename, $images)
    {
        $import_file = "<?php\n\nif (!defined('IN_PHPBB'))\n{\n	exit;\n}\n\n";
        ...
        $import_file .= "\$image_name = '" . str_replace("'", "{{$import_schema}}", (string) $image_name) . " ';\n";
        $import_file .= "\$filename = " . (($filename) ? 'true' : 'false') . ";\n";
        $import_file .= "\$user_data = array(\n";
        $import_file .= "	'user_id'		=> " . $user_row['user_id'] . ",\n";
        // We add a space at the end of the name, to not get troubles with \',
        $import_file .= "	'username'		=> '" . str_replace("'", "{{$import_schema}}", (string) $user_row['username']) . " ',\n";
        ...
        foreach ($images as $image_src) {
            $import_file .= "	'" . str_replace($replace_chars, $replace_with, (string) $image_src) . "',\n";
        }
    ...
    public function log_import_error($import_schema, $error)
    {
    ...
    public function cleanup()
    {
        ...
        if ($prune && empty($prune_pattern)) {
            ...
            if (isset($_POST['prune_time_check'])) {
                $prune_time = explode('-', (string) request_var('prune_time', ''));
        ...
        } elseif ($delete || $prune || (isset($_POST['cancel']))) {
