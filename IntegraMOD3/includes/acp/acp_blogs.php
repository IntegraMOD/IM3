...
class acp_blogs
{
	...
	*/
	...
	function plugins($id, $mode)
	{
		...
		foreach (blog_plugins::$available_plugins as $name => $data)
		{
			...
			if ($installed)
			{
				...
				{
					$version = array('files' => explode('.', (string) $data['plugin_version']), 'db' => explode('.', (string) blog_plugins::$plugins[$name]['plugin_version']));
	...
	function search_settings($id, $mode)
	{
		...
		foreach ($search_types as $type)
		{
			...
			$name = ucfirst(strtolower(str_replace('_', ' ', (string) $type)));
		...
		foreach ($settings as $config_name => $var_type)
		{
			...
			$var_type = explode(':', (string) $var_type);
	...
	function search_index($id, $mode)
	{
		...
		$this->state = explode(',', (string) $config['search_indexing_state']);
		...
		if ($action)
		{
			...
			$name = ucfirst(strtolower(str_replace('_', ' ', (string) $this->state[0])));
		...
		$template->assign_vars(array(
			...
			'UA_PROGRESS_BAR'		=> addslashes((string) append_sid("{$phpbb_admin_path}index.$phpEx", "i=$id&amp;mode=$mode&amp;action=progress_bar")),
		));
	...
	*/
	function init_search($type, &$search, &$error)
	{
		...
		if (!preg_match('#^\w+$#', (string) $type) || !file_exists("{$phpbb_root_path}blog/search/$type.$phpEx"))
		{
	...
	function extensions($id, $mode)
	{
		...
		if ($submit)
		{
			...
			if (!sizeof($error))
			{
				...
				$max_filesize	= ($size_select == 'kb') ? round($max_filesize * 1024) : (($size_select == 'mb') ? round($max_filesize * 1_048_576) : $max_filesize);
				...
				add_log('admin', 'LOG_ATTACH_EXTGROUP_' . strtoupper((string) $action), $group_name);
			}
			...
			if (!sizeof($error))
			{
				$notify[] = $user->lang['SUCCESS_EXTENSION_GROUP_' . strtoupper((string) $action)];
			}
		}
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				$forum_ids = (!$ext_group_row['allowed_forums']) ? array() : unserialize(trim((string) $ext_group_row['allowed_forums']));
			...
			case 'add':
				...
				$size_format = ($ext_group_row['max_filesize'] >= 1_048_576) ? 'mb' : (($ext_group_row['max_filesize'] >= 1024) ? 'kb' : 'b');

				$ext_group_row['max_filesize'] = ($ext_group_row['max_filesize'] >= 1_048_576) ? round($ext_group_row['max_filesize'] / 1_048_576 * 100) / 100 : (($ext_group_row['max_filesize'] >= 1024) ? round($ext_group_row['max_filesize'] / 1024 * 100) / 100 : $ext_group_row['max_filesize']);
				...
				if (sizeof($imglist))
				{
					...
					foreach ($imglist as $key => $img)
					{
						...
						if (strlen((string) $img) > 255)
						{
							continue;
						}

						$filename_list .= '<option value="' . htmlspecialchars((string) $img) . '"' . $selected . '>' . htmlspecialchars((string) $img) . '</option>';
					}
				}
				...
				$template->assign_vars(array(
					...
					'L_LEGEND'			=> $user->lang[strtoupper((string) $action) . '_EXTENSION_GROUP'])
				);
