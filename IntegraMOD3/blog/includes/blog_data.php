	...
	public function get_user_data($id, $user_queue = false, $username = false)
	{
		...
		if ($username)
		{
			if (isset($user_id) && $user_id != ANONYMOUS)
			{
				...
				self::$user[$user_id]['status'] = (is_null($status_data[$user_id]) && time() - $update_time < $status_data[$user_id]['online_time'] && (($status_data[$user_id]['viewonline'] && $row['user_allow_viewonline']) || $auth->acl_get('u_viewonline'))) ? true : false;
				unset($status_data);
