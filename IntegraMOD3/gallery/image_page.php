...
$template->assign_vars(array(
    ...
    'UC_IMAGE'          => phpbb_gallery_image::generate_link('medium', phpbb_gallery_config::get('link_imagepage'), $image_id, $image_data['image_name'], $album_id, ((substr($image_data['image_filename'], -3) == 'gif') ? true : false), false, '', (isset($next_data['image_id'])) ? $next_data['image_id'] : 0),
	...
	if ($image_data['image_comments'] > 0)
	{
		...
		foreach ($comments as $row)
		{
			...
			$template->assign_block_vars('commentrow', array(
				...
				'U_FB'			=> $user_cache[$user_id]['fb'],
				'U_IG'			=> $user_cache[$user_id]['ig'],
				'U_PT'			=> $user_cache[$user_id]['pt'],
				'U_TWR'			=> $user_cache[$user_id]['twr'],
				'U_SKP'			=> $user_cache[$user_id]['skp'],
				'U_TG'			=> $user_cache[$user_id]['tg'],
				'U_LI'			=> $user_cache[$user_id]['li'],
				'U_TT'			=> $user_cache[$user_id]['tt'],
				'U_DC'			=> $user_cache[$user_id]['dc'],			
...
if (phpbb_gallery::$auth->acl_check('m_status', $album_id, $album_data['album_user_id']) || ($image_data['image_contest'] != phpbb_gallery_image::IN_CONTEST))
{
	...
	$template->assign_vars(array(
		...
		'U_POSTER_FB'			=> $user_cache[$user_id]['fb'],
		'U_POSTER_IG'			=> $user_cache[$user_id]['ig'],
		'U_POSTER_PT'			=> $user_cache[$user_id]['pt'],
		'U_POSTER_TWR'			=> $user_cache[$user_id]['twr'],
		'U_POSTER_SKP'			=> $user_cache[$user_id]['skp'],
		'U_POSTER_TG'			=> $user_cache[$user_id]['tg'],
		'U_POSTER_LI'			=> $user_cache[$user_id]['li'],
		'U_POSTER_TT'			=> $user_cache[$user_id]['tt'],
		'U_POSTER_DC'			=> $user_cache[$user_id]['dc'],
