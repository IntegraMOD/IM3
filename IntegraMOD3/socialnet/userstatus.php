...
if (!class_exists('socialnet_userstatus')) {
    class socialnet_userstatus
    {
         ...
         * Insert staus to DB
         */
        public function _status_share($on_the_wall = false)
        {
            ...
            if (trim($new_status) != '') {
                ...
                $mentions = html_entity_decode((string) $mentions);
                $mentions = json_decode($mentions, null, 512, JSON_THROW_ON_ERROR);
                ...
                if (!empty($mentions)) {
                    ...
                    foreach ($mentions as $idx => $mention) {
                        $mentions_users[] = htmlentities((string) $mention->value);
                    }
                    // Transform MENTIONS.
                ...
                if ($isPage) {
                    ...
                    $page['title'] = htmlspecialchars_decode((string) $page['title'], ENT_QUOTES);
                    $page['desc'] = htmlspecialchars_decode((string) $page['desc'], ENT_QUOTES);
         ...
         * Delete User Status
         */
        public function _status_delete()
        {
            global $db, $auth, $user;
         ...
         * Get Activity
         */
        public function _get_activity(): never
        {
            global $template;
            ...
            $return['content'] = str_replace(array('<a ', '</a>', ), array('<span ', '</span>', ), (string) $this->p_master->get_page());
            $return['content'] = preg_replace('/<img[^>]*>/si', '', $return['content']);

            header('Content-type: application/json');
            header("Cache-Control: no-cache, must-revalidate");
            header("Expires: Sat, 26 Jul 1997 05:00:00 GMT");
            die(json_encode($return, JSON_THROW_ON_ERROR));
        }

        /**
         * Delete Activity
         */
        public function _delete_activity()
        {
            global $db, $auth, $user;
        ...
        public function _status_one(): never
        {
         ...
         * Add new comment
         */
        public function _comment_share()
        {
         ...
         * Load more comments
         */
        public function _comment_more(): never
        {
            $user_id = request_var('u', ANONYMOUS);
            $status_id = request_var('s_id', 0);
        ...
        public function _get_status(): never
        {
            global $template;
         ...
         * Load last status
         */
        public function _get_last_userstatus()
        {
         ...
         * Load last status
         */
        public function _get_last_status($user_id, $user_status = 0)
        {
         ...
         */
        public function _get_statuses($user_id, $last_status_id = 0, $status_limit = 10, $comment_limit = 3, $only_one = false)
        {
         ...
         */
        public function _get_status_array($status_row, $my_friends = array(), $comment_limit = 3)
        {
            ...
            if ($status_row['page_data'] != '') {
                ...
                $pageData['video'] = html_entity_decode((string) $pageData['video']);
                ...
                $pageData['title_title'] = str_replace('"', '', (string) $pageData['title']);

                if ($pageData['video'] == '') {
                    $parsed_url = array();
                    $parsed_url = parse_url((string) $pageData['url']);
                    $pageData['server_url'] = $parsed_url['scheme'] . '://' . $parsed_url['host'];
                }

                foreach ($pageData as $key => $value) {
                    $template_block_data['PAGE_' . strtoupper((string) $key)] = $value;
                }
        ...
        public function _get_mention()
        {
            global $db, $user;

            $uname = request_var('uname', '', true);
            $uname_clean = strtolower((string) utf8_clean_string($uname));
            ...
            while ($row = $db->sql_fetchrow($rs)) {
                ...
                $row['username'] = html_entity_decode((string) $row['username']);
