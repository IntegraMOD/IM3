    ...
    class socialnet_im
    {
        ...
        public function init()
        {
            ...
            $template_assign_vars = array_merge($template_assign_vars, array(
                ...
                'S_SN_USERNAME'				 => addslashes((string) $user->data['username']),
        ...
        public function load($mode)
        {
            ...
            die(json_encode($this->items, JSON_THROW_ON_ERROR));
        }
        ...
        public function openChatBox()
        {
            ...
            $usernameTo = strip_tags((string) request_var('usernameTo', '', true));
            ...
            $b_no_avatar_me = stripos((string) $this->config['my_avatar'], 'socialnet/no_avatar') !== false ? true : false;
            $b_no_avatar_sender = stripos((string) $userto_avatar, 'socialnet/no_avatar') !== false ? true : false;
        ...
        public function _startIM($mode = '')
        {
            ...
            for ($i = 0; isset($chatBoxRowSet[$i]); $i++) {
                $row = $chatBoxRowSet[$i];
                $status = $this->items['onlineUsers'][$row['uid_to']]['status'] ?? 0;
                ...
                $template->assign_block_vars('sn_im_chatbox', array(
                    ...
                    'USERNAME_TO_NO_COLOR'	 => html_entity_decode((string) $row['username_to']),
                    ...
                    if (!empty($in_group)) {
                        foreach ($in_group as $user_id => $usr) {
                            $template->assign_block_vars('sn_im_online_ufg.user', array(
                                ...
                                'USERCLEAN'	 => addslashes((string) $usr['userClean']),
                                'ONLINE'	 => $usr['online'],
                            ));
         ...
         * @return void
         */
        public function _writeMessage($message, $uid)
        {
            ...
            $template->assign_block_vars('sn_im_chatbox.message', array(
                ...
                'MESSAGE'		 => $message,
                'B_NO_AVATAR'	 => stripos((string) $this->config['my_avatar'], 'socialnet/no_avatar') !== false ? true : false,
                'TIME'			 => $message_time,
         ...
         * @return void
         */
        public function _markRecievedMessages()
        {
            global $user, $db;
         ...
         * @return void
         */
        public function _snImLogout($enable)
        {
            global $db, $user;
         ...
         * @return void
         */
        public function _snImSound($enable)
        {
            global $db, $user;
         ...
         * @return void
         */
        public function _snImUserGroup($hide)
        {
            global $db, $user;
        ...
        public function _open_chatbox_avatar($uidTo)
        {
        ...
        public function _keyboardString($sequence = array())
        {
            $closeKey = '';
            foreach ($sequence as $idx => $allow) {
                ...
                if ($allow) {
                    ...
                    $closeKey .= strtoupper((string) $idx);
                }
            }
        ...
        public function _displaySmilies()
        {
            ...
            if (sizeof($smilies)) {
                ...
                foreach ($smilies as $row) {
                    $template->assign_block_vars('im_smiley', array(
                        ...
                        'A_SMILEY_CODE'	 => addslashes((string) $row['code']),
