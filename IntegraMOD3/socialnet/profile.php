    ...
    class socialnet_profile
    {
        ...
        public function init()
        {
            ...
            for ($i = 1; $i <= 12; $i++) {
                ...
                $monthNamesShort .= "'" . addslashes((string) $user->lang['datetime'][$nM[1]]) . "'";
                $monthNames .= "'" . addslashes((string) $user->lang['datetime'][$nM[0]]) . "'";
            }
            ...
            $template_assign_vars = array(
                ...
                'S_SN_UP_EMOTES_ENABLED'	 => $this->p_master->config['up_emotes'] ?? 0,
        ...
        public function tab_info($user_id)
        {
            ...
            if ($config['allow_birthdays'] && $member['user_birthday']) {
                [$bday_day, $bday_month, $bday_year] = array_map('intval', explode('-', (string) $member['user_birthday']));
            ...
            if ($member['user_birthday']) {
                $birthday_arr = array_map('intval', explode('-', (string) $member['user_birthday']));
        ...
        public function tab_friends($user_id)
        {
        ...
        public function tab_stats($user_id)
        {
            global $phpbb_root_path, $phpEx;
            ...
            foreach ($profile_groups as $row) {
                ...
                } elseif (!$auth_hidden_groups && $row['group_type'] == GROUP_HIDDEN && !isset($user_groups[$row['group_id']])) {
            ...
            $template->assign_vars(array(
                ...
                'WARNINGS'			 => $member['user_warnings'] ?? 0,
        ...
        public function tab_wall($user_id)
        {
            return '';
        }

        public function tab_report_user($user_id)
        {
            ...
            if ($submit && $reason_id) {
                ...
                if (!$row || (!$report_text && strtolower((string) $row['reason_text']) == 'other')) {
        ...
        public function last_profile_change()
        {
            ...
            if ($submit && isset($modules_[$module]) && in_array($mode, $modules_[$module])) {
                switch ($mode) {
                    ...
                    case 'signature':
                        ...
                        $original['user_sig'] = preg_replace('/\[[^]]*\]/si', '', (string) $user->data['user_sig']);
                        $data['user_sig'] = preg_replace('/\[[^]]*\]/si', '', (string) utf8_normalize_nfc(request_var('signature', (string) $original['user_sig'], true)));
                    ...
                    case 'avatar':
                        ...
                        if (isset($_FILES['uploadfile']) && $_FILES['uploadfile']['error'] == 0) {
                            ...
                            $aw = $image[0];
                            $ah = $image[1];
                        } elseif (isset($_FILES['uploadfile']) && $_FILES['uploadfile']['error'] != 0 && $_FILES['uploadfile']['name'] != '') {
                            return;
                        } elseif (isset($data['uploadurl']) && !empty($data['uploadurl'])) {
                    ...
                    case 'reg_details':
                        ...
                        $data = array(
                            ...
                            'user_email' => strtolower((string) request_var('email', $user->data['user_email'])),
                        );
        ...
        public function emote($user_id)
        {
            ...
            die(json_encode($send_emote, JSON_THROW_ON_ERROR));
        }
        ...
        public function upEdit($user_id)
        {
            ...
            $fieldSet = explode('-', (string) $fieldName);
            $tableSH = $fieldSet[0];
            $field = $fieldSet[1];
            ...
            if ($b_bbcode) {
                ...
                for ($i = 1; $i <= 12; $i++) {
                    ...
                    if (str_contains((string) $value, (string) $user->lang['datetime'][$month])) {
                        break;
                    }
                }

                $gb_value = str_replace($user->lang['datetime'][$month], $month, (string) $value);
        ...
        public function _prepare_for_entry(&$changed)
        {
            global $user;
        ...
        public function _parse_dateFormat_to_js($format)
        {
            $replace = array(
            ...
            return str_replace(array_keys($replace), $replace, (string) $format);
        }
    }
}
...
if (!function_exists('hook_template_userstatus_profile_array_callback')) {
    function hook_template_userstatus_profile_array_callback(&$item, $key)
    {
        ...
        if (preg_match($preg_match_profile, (string) $item)) {
            $item = preg_replace($preg_match_profile, 'profile.' . $phpEx . '?\3\4', (string) $item);
        }
    }
}
...
if (!function_exists('profile_change_cut_string')) {
    function profile_change_cut_string(&$value)
    {
        ...
        if (strlen((string) $value) > $config['ap_max_profile_value']) {
            $value = truncate_string($value, $config['ap_max_profile_value'] + 1);
            if (strrpos((string) $value, ' ') != 0) {
