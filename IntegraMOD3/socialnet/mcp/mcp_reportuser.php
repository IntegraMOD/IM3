...
class mcp_reportuser
{
  ...
  */
    public function close_user_report($report_id_list, $action)
    {
        ...
        if (confirm_box(true)) {
            ...
            confirm_box(false, $user->lang['SN_UP_'.strtoupper((string) $action) . "_REPORT" . ((sizeof($report_id_list) == 1) ? '' : 'S') . '_CONFIRM'], $s_hidden_fields);
        }
        ...
        $message = $user->lang['SN_UP_'.strtoupper((string) $action).'_REPORT'.((sizeof($report_id_list) == 1) ? '' : 'S').'_SUCCESS'] . '<br /><br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $redirect . '">', '</a>');
