...
class ucp_approval
{
    ...
    public function friends($id, $mode)
    {
        ...
        if ($submit) {
            ...
            $var_ary = array(
                ...
                'add'					 => '',
                'redirect'				 => base64_encode((string) append_sid("{$phpbb_root_path}activitypage.{$phpEx}")),
            );

            foreach ($var_ary as $var => $default) {
                $data[$var] = request_var($var, $default, true);
            }
            $data['redirect'] = base64_decode((string) $data['redirect']);
        ...
        $s_hidden_fields = build_hidden_fields(array('redirect' => request_var('redirect', base64_encode((string) $this->p_master->u_action))));
     ...
     */
    public function _friends_foes_add($id, $mode, &$data, &$error)
    {
        ...
        if (sizeof($data['add']) < $n) {
            $error[] = $user->lang['SN_FAS_NOT_ADDED_FRIENDS_IN_APPROVAL'] ?? '{ SN_FAS_NOT_ADDED_FRIENDS_IN_APPROVAL }';
        }
        ...
        if (sizeof($data['add']) < $n) {
            $error[] = $user->lang['SN_FAS_NOT_ADDED_FRIENDS_IN_FRIENDS'] ?? '{ SN_FAS_NOT_ADDED_FRIENDS_IN_FRIENDS }';
        }
        ...
        if (sizeof($data['add']) < $n) {
            $error[] = $user->lang['SN_FAS_NOT_ADDED_FRIENDS_IN_FOES'] ?? '{ SN_FAS_NOT_ADDED_FRIENDS_IN_FOES }';
        }
        ...
        if (sizeof($data['add']) < $n) {
            $error[] = $user->lang['NOT_ADDED_' . $l_mode . '_SELF'] ?? '{ NOT_ADDED_' . $l_mode . '_SELF }';
        }
