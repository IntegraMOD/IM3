...
class ucp_profile
{
                        ...
                        if ($submit) {
                            ...
                            if ($data['family_status'] && !empty($data['family_user'])) {
                                ...
                                if (!sizeof($error)) {
	                                ...
	                                if (!isset($family['user_id']) || !is_array($family['user_id'])) {
    ...
    public function approve_relation($approve = true)
    {
        ...
        if (confirm_box(true)) {
            ...
            if ($approve) {
                if ($row['family']) {
                    ...
                    if ($row['anniversary']) {
                        $relationship_arr = array_map('intval', explode('-', (string) $row['anniversary']));
                        $relationship_anniversary = $user->format_date(gmmktime(0, 0, -$user->timezone, (int) $relationship_arr[1], (int) $relationship_arr[0], (int) $relationship_arr[2]), '|j. F Y|');
                    }
