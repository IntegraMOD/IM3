    ...
    class socialnet_approval
    {
        ...
        public function group()
        {
            ...
            if (($gid == 0 && $tid == '') || $uid == 0 || $sub == '') {
                exit(json_encode(array('error' => 1, 'text' => "some user problem occured\nReload page using F5 or CTRL+F5")));
            }
            ...
            exit(json_encode(array('error' => 0, 'gid' => $gid, 'uid' => $uid, 'sub' => $sub, 'user' => $user->data['user_id']), JSON_THROW_ON_ERROR));
        }
        ...
        public function _group_load()
        {
            ...
            header("Cache-Control: no-cache, must-revalidate");
            header("Expires: Sat, 26 Jul 1997 05:00:00 GMT");
            exit(json_encode(array('html' => $html), JSON_THROW_ON_ERROR));
        }
