...
if (!class_exists('socialnet_approval')) {
    class socialnet_approval
    {
        ...
        public function load()
        {
            ...
            match ($mode) {
                'memberlist' => $template->set_filenames(array('body' => "socialnet/memberlist_viewprofile_friends.html")),
                'mutual' => $template->set_filenames(array('body' => "socialnet/memberlist_viewprofile_mutual.html")),
                default => $template->set_filenames(array('body' => "socialnet/ucp_approval_block_{$mode}.html")),
            };
        ...
        public function group()
        {
            ...
            die(json_encode(array('error' => 0, 'gid' => $gid, 'uid' => $uid, 'sub' => $sub, 'user' => $user->data['user_id']), JSON_THROW_ON_ERROR));
        }

        public function _group_load()
        {
            ...
            header("Cache-Control: no-cache, must-revalidate");
            header("Expires: Sat, 26 Jul 1997 05:00:00 GMT");
            die(json_encode(array('html' => $html), JSON_THROW_ON_ERROR));
        }
...
if (!function_exists('hook_template_approval_array_callback')) {
    function hook_template_approval_array_callback(&$item, $key)
    {
        ...
        if (preg_match($preg_match_profile, (string) $item) && !preg_match('/mode=foes/si', (string) $item)) {
            $item = preg_replace($preg_match_profile, 'ucp.' . $phpEx . '?i=socialnet&amp;mode=module_approval_friends\2', (string) $item);
        }
    }
}
