...
class acp_approval extends socialnet
{
     ...
     * @return void
     */
    public function acp_sett_modules($enabled)
    {
        ...
        if ($enabled) {
            ...
            $cache->put('_sn_module_' . static::class, array('count' => count($zebra_1), 'data' => $zebra_1));
     ...
     * Inicializace modulu
     */
    public function acp_initialize()
    {
        global $cache, $db, $template, $starttime, $phpbb_root_path, $phpEx, $user;

        $data = $cache->get($this->p_master->initModuleCacheName . static::class);
        $cache->destroy($this->p_master->initModuleCacheName . static::class);
        ...
        if (!empty($data['data'])) {
            foreach ($data['data'] as $idx => $zebra) {
                ...
                if ($endtime[1] + $endtime[0] - $this->p_master->initModuleMaxTime > $starttime) {
                    $cache->put($this->p_master->initModuleCacheName . static::class, $data);
     ...
     * @return void
     */
    public function send_pm($send_from, $send_to)
    {
