...
if ($urldata['host'] == 'www.youtube.com') {
    ...
    [$string, $status] = get_remote_page($url, $urldata);

    if ($status != "200") {
        parse_str((string) $urldata['query'], $query);

        $ytData['url'] = 'http://gdata.youtube.com/feeds/api/videos?max-results=1&q=' . $query['v'];

        [$string, $status] = get_remote_page($ytData['url'], parse_url((string) $ytData['url']));
        $url = $urlOrig;
    }
} else {
    [$string, $status] = get_remote_page($url, $urldata);
 ...
 */
$ytData = array();
if (preg_match("/^Couldn\'t resolve host/s", (string) $string)) {
    $status = 502;
}
...
$is_file = preg_match('/\.(' . substr($extensions, 1) . ')$/i', (string) $urldata['path']);
...
if ($img = @imagecreatefromstring($string)) {
     ...
     */
    // Title
    preg_match_all('/<media:title[^>]*>(.*)<\/media:title>/si', (string) $string, $match);
    $url_title = html_entity_decode((string) $match[1][0]);
    // DESC
    preg_match_all('/<media:description[^>]*>(.*)<\/media:description>/si', (string) $string, $match);
    $url_desc = html_entity_decode((string) $match[1][0]);

    preg_match_all('/<media:thumbnail url=\'([^\']*)\' height=\'([0-9]*)\' width=\'([0-9]*)\'[^>]*>/si', (string) $string, $match);
     ...
     * Is it file?
     */
    preg_match('/<meta http-equiv="Content-Type" content="text\/html; charset=([^"]*)" ?\/?>/si', (string) $string, $match);
    ...
    preg_match_all($title_regex, (string) $string, $title, PREG_PATTERN_ORDER);
     ...
     */
    $content = $string;
    $content = preg_replace("'<style[^>]*>.*</style>'siU", '', (string) $content); // strip js
    ...
    preg_match_all($image_regex, (string) $string, $img, PREG_PATTERN_ORDER);
    ...
    if ($embevi->parseUrl($url)) {
        ...
        $video = html_entity_decode((string) $video);
    ...
    for ($i = 0; isset($images_array[$i]); $i++) {
        if ([$width, $height, $type, $attr] = @getimagesize($images_array[$i])) {
