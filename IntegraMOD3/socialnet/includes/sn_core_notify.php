...
class sn_core_notify
{
     ...
     * @param array $data Notification data to be displayed
     */
    public function prepare_add_sql($type, $to_user, $data)
    {
        global $user;
     ...
     * Send PM
     */
    public function send_pm($type, $send_to, $data)
    {
        ...
        $lang_text = $data['text'];
        $lang_subject = $lang[$lang_text . '_PM_TITLE'] ?? "{ {$lang_text}_PM_TITLE }";
        ...
        $lang_message = $lang[$lang_text] ?? "{ {$lang_text} }";
        unset($data['text']);
