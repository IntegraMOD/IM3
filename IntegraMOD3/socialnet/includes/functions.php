...
class snFunctions
{
     ...
     * @return	array	list of online users
     */
    public function obtain_users_online(mixed $time) //sn_core_users
    {
        global $db, $user;
     ...
     */
    public function fms_users_sqls(mixed $mode, mixed $user_id) //sn_core_users
    {
     ...
     * @return	array
     */
    public function _fms_users_fill($rowset, mixed $user_id_field, $limit, mixed $avatar_size, $add_friend_link, $tpl_name) //sn_core_users
    {
     ...
     */
    public function format_date($part, mixed $gmepoch, $format = false, $forcedate = false)
    {
        global $user;
    ...
    public function users_autocomplete()
    {
        ...
        header("Cache-Control: no-cache, must-revalidate");
        header("Expires: Sat, 26 Jul 1997 05:00:00 GMT");
        die(json_encode($return_arr, JSON_THROW_ON_ERROR));
    }
        ...
        if (isset($_POST['login'])) {
            ...
            switch ($result['status']) {
                ...
                case LOGIN_ERROR_PASSWORD_CONVERT:
                    $err = sprintf($user->lang[$result['error_msg']], ($this->config['email_enable']) ? '<a href="' . append_sid("{$phpbb_root_path}ucp.$phpEx", 'mode=sendpassword') . '">' : '', ($this->config['email_enable']) ? '</a>' : '', ($this->config['board_contact']) ? '<a href="mailto:' . htmlspecialchars((string) $this->config['board_contact']) . '">' : '', ($this->config['board_contact']) ? '</a>' : '');
                    break;
    ...
    public function birthday()
    {
        ...
        if (empty($friends_birthday)) {
            ...
            $order_by = match ($db->sql_layer) {
                'mssql', 'mssql_odbc' => 'u.user_birthday ASC',
                default => 'SUBSTRING(u.user_birthday FROM 4 FOR 2) ASC, SUBSTRING(u.user_birthday FROM 1 FOR 2) ASC, u.username_clean ASC',
            };
    ...
    public function relationship_status($status_id, $approved = false)
    {
        ...
        $status = match ($status_id) {
            '1' => $user->lang['SN_UP_SINGLE'],
            '2' => $user->lang['SN_UP_IN_RELATIONSHIP'] . (($approved) ? ' ' . $user->lang['SN_UP_WITH'] : ''),
            '3' => $user->lang['SN_UP_ENGAGED'] . (($approved) ? ' ' . $user->lang['SN_UP_TO'] : ''),
            '4' => $user->lang['SN_UP_MARRIED'] . (($approved) ? ' ' . $user->lang['SN_UP_TO'] : ''),
            '5' => $user->lang['SN_UP_ITS_COMPLICATED'] . (($approved) ? ' ' . $user->lang['SN_UP_WITH'] : ''),
            '6' => $user->lang['SN_UP_OPEN_RELATIONSHIP'] . (($approved) ? ' ' . $user->lang['SN_UP_WITH'] : ''),
            '7' => $user->lang['SN_UP_WIDOWED'],
            '8' => $user->lang['SN_UP_SEPARATED'],
            '9' => $user->lang['SN_UP_DIVORCED'],
            default => '',
        };
    ...
    public function family_status($status_id)
    {
        ...
        $status = match ($status_id) {
            '20' => $user->lang['SN_UP_SISTER'],
            '21' => $user->lang['SN_UP_BROTHER'],
            '22' => $user->lang['SN_UP_DAUGHTER'],
            '23' => $user->lang['SN_UP_SON'],
            '24' => $user->lang['SN_UP_MOTHER'],
            '25' => $user->lang['SN_UP_FATHER'],
            '26' => $user->lang['SN_UP_AUNT'],
            '27' => $user->lang['SN_UP_UNCLE'],
            '28' => $user->lang['SN_UP_NIECE'],
            '29' => $user->lang['SN_UP_NEPHEW'],
            '30' => $user->lang['SN_UP_COUSIN_FEMALE'],
            '31' => $user->lang['SN_UP_COUSIN_MALE'],
            '32' => $user->lang['SN_UP_GRANDDAUGHTER'],
            '33' => $user->lang['SN_UP_GRANDSON'],
            '34' => $user->lang['SN_UP_GRANDMOTHER'],
            '35' => $user->lang['SN_UP_GRANDFATHER'],
            '36' => $user->lang['SN_UP_SISTER_IN_LAW'],
            '37' => $user->lang['SN_UP_BROTHER_IN_LAW'],
            '38' => $user->lang['SN_UP_MOTHER_IN_LAW'],
            '39' => $user->lang['SN_UP_FATHER_IN_LAW'],
            '40' => $user->lang['SN_UP_DAUGHTER_IN_LAW'],
            '41' => $user->lang['SN_UP_SON_IN_LAW'],
            default => '',
        };
     ...
     */
    public function return_family(mixed $status_id, mixed $sex)
    {
     ...
     * @return
     */
    public function getCookie(mixed $name, mixed $default)
    {
        global $config;
        /*
 ...
 * @return	string
 */
function snFunctions_absolutePath(mixed $matches)
{
 ...
 * @return
 */
function snFunctions_absolutePathString(mixed $string)
{
...
if (!function_exists('json_encode')) {
     ...
     */
    function json_encode(mixed $data)
    {
        if (is_array($data) || is_object($data)) {
            ...
            if ($islist) {
                ...
                foreach ($data as $key => $value) {
                    $items[] = json_encode("$key", JSON_THROW_ON_ERROR) . ':' . json_encode($value, JSON_THROW_ON_ERROR);
                }
                $json = '{' . implode(',', $items) . '}';
            }
...
if (!function_exists('array_walk_recursive')) {
     ...
     */
    function array_walk_recursive(&$input, $funcname, mixed $userdata = '')
    {
