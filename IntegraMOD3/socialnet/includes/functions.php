...
class snFunctions
{
     ...
     */
    public $onlineUsersLoaded = false;
     ...
     */
public $fms_users_default = array(
	'mode'				 => 'friend',
	'user_id'			 => 0,
	'fmsf'				 => 0,
	'limit'				 => 0,
 ...
 */
	public function onlineUsers($all_online = false) //sn_core_users
	{
	 ...
	 * @param bool$all_online
	 *
	 * @globalobject  $dbphpBB database object
	 * @globalobject  $template  phpBB template object
	 * @globalobject  $user  phpBB user object
	 * @globalstring  $phpbb_root_path   phpBB root path string
	 *
	 * @returnarray   list of online users
	 */
	public function onlineSelect($all_online = false) //sn_core_users
	{
	 ...
	 * @returnarray   list of online users
	 */
	public function obtain_users_online($time) //sn_core_users
	{
	global $db, $user;
	 ...
	 */
	public function online_users($json = false) // sn_core_users -> online
	{
	...
	die(json_encode(array('onlineUsers' => $rows, 'user_online' => $user->data['user_im_online'], 'list' => $onlineList), JSON_THROW_ON_ERROR));
	}
	}
	 ...
	 * @globalobject  $user  phpBB user object
	 * @globalstring  $phpbb_root_path   phpBB root path string
	 * @globalobject  $cache phpBB cache object
	 * @returnarray   array of SQLs
	 */
	public function fms_users_sqls($mode, $user_id) //sn_core_users
	{
	 ...
	 * @property booleanis_not_empty- Is not empty current block
	 *
	 ...
	 *
	 */
	public function fms_users($params = array()) // sn_core_users -> get_fms
	{
	...
	$lmode = strtoupper((string) $mode);
	$lmodeshort = strtoupper((string) $mode_short);
	if (!$block_content['is_not_empty']) {
	$block_content['content'] = $user->lang['FAS_' . $lmodeshort . '_NO_TOTAL'] ?? '{ FAS ' . $lmodeshort . ' NO TOTAL }';
	$pagination = '';
	}
	 ...
	 * @return
	 */
	public function _fms_users_pagination($mode, $total, $start, $limit, $user_id, $tpl_name, $profile_link = true) // sn_core_users
	{
	 ...
	 * @globalobject  $template  phpBB template object
	 * @globalstring  $phpbb_root_path   phpBB root path string
	 * @globalstring  $phpEx php extension
	 * @globalarray   $configphpBB config array
	 * @globalobject  $user  phpBB user object
	 *
	 * @returnarray
	 */
	public function _fms_users_fill($rowset, $user_id_field, $limit, $avatar_size, $add_friend_link, $tpl_name) //sn_core_users
	{
	 ...
	 */
	public function format_date($part, $gmepoch, $format = false, $forcedate = false)
	{
	global $user;
	...
	$formatUser = trim(preg_replace('/[' . $timeChars . ']([ ,.:-\\\|]|$)/s', '', (string) $user->data['user_dateformat']));
	...
	$row_array['value'] = html_entity_decode((string) $row['username']);
	 ...
	 * @access public
	 *
	 * @param  string  $block_nameName of block that should be loaded
	 *
	 ...
	 * @access public
	 *
	 * @param array   $blocksName of blocks that should be loaded
	 *
	 * @returnarray   results of called block method
	 */
	 ...
	 * @globalobject  $template  phpBB template object
	 * @globalobject  $dbphpBB database object
	 * @globalobject  $auth  phpBB auth object
	 *
	 * @return bool
	 */
	...
	$err = sprintf($user->lang[$result['error_msg']], ($this->config['email_enable']) ? '<a href="' . append_sid("{$phpbb_root_path}ucp.$phpEx", 'mode=sendpassword') . '">' : '', ($this->config['email_enable']) ? '</a>' : '', ($this->config['board_contact']) ? '<a href="mailto:' . htmlspecialchars((string) $this->config['board_contact']) . '">' : '', ($this->config['
	board_contact']) ? '</a>' : '');
	break;
	...
	$err = (!$this->config['board_contact']) ? sprintf($user->lang[$result['error_msg']], '', '') : sprintf($user->lang[$result['error_msg']], '<a href="mailto:' . htmlspecialchars((string) $this->config['board_contact']) . '">', '</a>');
	}
...
/**
	...
	$order_by = $db->sql_layer === 'mssql' || $db->sql_layer === 'mssql_odbc' ? 'u.user_birthday ASC' : 'SUBSTRING(u.user_birthday FROM 4 FOR 2) ASC, SUBSTRING(u.user_birthday FROM 1 FOR 2) ASC, u.username_clean ASC';
	...
	$arr_birthday = explode('-', (string) $row['user_birthday']);
	 ...
	 * @access public
	 *
	 * @paramint $limit display specified amount of suggestions
	 *
	 * @globalobject  $dbphpBB database object
	 * @globalobject  $template  phpBB template object
	 * @globalstring  $phpbb_root_path   phpBB root path string
	 ...
	 * @globalobject  $user  phpBB user object
	 * @globalobject  $cache phpBB cache object
	 * @globalobject  $socialnet Socialne object
	 */
	public function friends_suggestions($limit = 4)
	{
	 ...
	 * @globalstring  $phpEx php extension
	 */
	public function recent_discussions()
	{
	 ...
	 * @access public
	 *
	 * @paramstring  $new_status status text
	 * @paramint $wall_id ID of user to whom wall status should be posted
	 *
	 * @globalobject  $socialnet Socialnet object
	 */
	public function post_status($new_status, $wall_id = 0)
	{
	global $socialnet;
	 ...
	 * @return
	 */
	public function _gen_menu($parent_id = 0)
	{
	 ...
	 * @globalobject  $user  phpBB user object
	 *
	 * @returnstring  language string for relationship status
	 */
	public function relationship_status($status_id, $approved = false)
	{
	global $user;
	 ...
	 * @globalobject  $user  phpBB user object
	 *
	 * @returnstring  language string for family status
	 */
	public function family_status($status_id)
	{
	global $user;
	 ...
	 * @globalobject  $socialnet Socialnet object
	 *
	 * @returnstring  list of HTML <option>s
	 */
	public function return_family($status_id, $sex)
	{
	 ...
	 * @globalarray   $configphpBB config array
	 */
	public function _calc_bbcodeFlags()
	{
	global $config;
	 
	$this->allow_bbcode = $config['sn_allow_bbcode'] ?? $this->allow_bbcode;
	$this->allow_urls = $config['sn_allow_urls'] ?? $this->allow_urls;
	$this->allow_smilies = $config['sn_allow_smilies'] ?? $this->allow_smilies;
	 ...
	 * @param boolean $globalTrue for checking a global permission setting, False for a local permission setting
	 *
	 * @globalobject  $dbphpBB database object
	 *
	 * @returnboolean true if it exists, false if not
	 */
	public function _permission_exists($auth_option, $global = true)
	{
	global $db;
	 ...
	 * @globalobject  $user  phpBB user object
	 */
	public function _version_checker($file)
	{
	...
	$xml_data = simplexml_load_string((string) $update_xml);
	...
	$pay = !empty($module['pay']);
	 
	if ($code_name == 'socialNet') {
	$not_up_to_date = version_compare($version, $config['version_socialNet'] ?? '0');
	 
	$template->assign_vars(array(
	'SN_VERSION_CHECK_NOT_UP_TO_DATE' => ($not_up_to_date == 1),
	'SN_VERSION_CHECK_DOWNLOAD' => $download,
	'SN_VERSION_CHECK_NAME' => $user->lang[$name] ?? $name,
	...
	'SN_VERSION_INSTALLED' => $config['version_socialNet'] ?? '0',
	...
	if ($status == 1) {
	$status += version_compare($version, $config['version_' . $code_name] ?? '0');
	}
	...
	'MSG' => ($msg != '') ? $msg : '',
	'STATUS' => $status,
	'INSTALLED_VERSION' => $config['version_' . $code_name] ?? '0',
	'DOWNLOAD' => $download,
	'B_PAY' => $pay,
	));
	}
	 ...
	 * @returnmixed
	 */
	public function _setup_array($xml, $get_attributes = 1, $priority = 'tag')
	{
...
/**
	 ...
	 * @param boolean $flattenValuesChoose wether to flatten values
	 *or to set them under a particular index.
	 *defaults to true;
	 * @param boolean $flattenAttributes Choose wether to flatten attributes
	 *or to set them under a particular index.
	 *Defaults to true;
	 * @param boolean $flattenChildrenChoose wether to flatten children
	 *or to set them under a particular index.
	 *Defaults to true;
	 * @param string $valueKeyindex for values, in case $flattenValues was set to
	 *false. Defaults to "@value"
	 * @param string $attributesKeyindex for attributes, in case $flattenAttributes was set to
	 *false. Defaults to "@attributes"
	 * @param string $childrenKeyindex for children, in case $flattenChildren was set to
	 *false. Defaults to "@children"
	 * @return array the resulting array.
	 */
	public function _simpleXMLToArray($xml, $flattenValues = true, $flattenAttributes = true, $flattenChildren = true, $valueKey = '@value', $attributesKey = '@attributes', $childrenKey = '@children')
	{
	$return = array();
	...
	$this->cookies[$name] = $this->request_var($cookieName, $default, false, true);
	}
	return $this->cookies[$name];
	}
	 
	/**
	 * Helper function to replace the deprecated request_var function
	 *
	 * @param string $var_name The variable name to retrieve
	 * @param mixed $default The default value
	 * @param bool $multibyte Whether to treat the variable as multibyte
	 * @param bool $cookie Whether to look in $_COOKIE or not
	 * @return mixed
	 */
	private function request_var($var_name, $default, $multibyte = false, $cookie = false)
	{
	...
	$result[$key] = $this->set_var($value, $default, $multibyte);
	}
	return $result;
	}
	 
	return $this->set_var($var, $default, $multibyte);
	}
	 
	/**
	 * Helper function to set variable types
	 *
	 * @param mixed $var The variable to set
	 * @param mixed $default The default value
	 * @param bool $multibyte Whether to treat as multibyte
	 * @return mixed
	 */
	private function set_var($var, $default, $multibyte = false)
	{
	if (is_null($var) || $var === '') {
	return $default;
	}
	 ...
	 *
	 *
	 * @globalstring  $phpbb_root_path   phpBB root path
	 * @globalarray   $configphpBB config array
	 * @globalobject  $user  phpBB user object
	 * @return string
	 */
	...
	// Windows server fix
	$_phpbb_root_path = str_replace('\\', '/', (string) $phpbb_root_path);
	$_script_path = str_replace('//', '/', str_replace('\\', '/', (string) $config['script_path']) . '/');
	$path = preg_replace('#^' . preg_quote($_phpbb_root_path) . '#si', $_script_path, (string) $string);
	 ...
	 * @since 22.12.2005
	 *
	 * @param array &$inputarray to be walked in
	 * @param callable$funcname  name of function to be called on every key of the array $input
	...
	$msg .= ($dg['file'] ?? 'unknown file') . ' - ' . ($dg['line'] ?? 'unknown line') . '<br />';
	$msg .= '</li>';
	}
	$msg .= '</ol>';
