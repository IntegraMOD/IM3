	...
	class socialnet extends snFunctions
{
    ...
    public $useTemplateHook = true;
    public $config = array();
    public $socialnet_root_path = '';
    public $modules = array();
    public $existing = array();
    public $modules_obj = array();
    public $bbCodeFlags = null;
    public $allow_bbcode = 1;
    public $allow_urls = 1;
    public $allow_smilies = 1;
    public $script_name = '';
 
    public $friendsCacheName = '_sn_Friends_';
    public $friendsCacheNameMutual = '_sn_MutualFriends_';
    public $groupsCacheName = '_sn_Groups_';
...
//      $this->snFunctions();
        ...
        $this->script_name = str_replace('.' . $phpEx, '', (string) $user->page['page_name']);
        $this->config = &$config;
        ...
        foreach ($rowset as $idx => $row) {
            ...
            if (preg_match('/^module_(.+)$/si', (string) $row['config_name'], $module_match)) {
                $moduleName = 'SN_' . strtoupper((string) $row['config_name']) . '_ENABLED';
            ...
            // SN BLOCKS
            if (preg_match('/^sn_block_(.*)$/si', (string) $row['config_name'], $block_match)) {
                $block_settings['B_' . strtoupper((string) $row['config_name']) . '_ENABLED'] = $row['config_value'];
            }
 
            // CONFIRM BOX
            if (preg_match('/^sn_cb(.+)$/', (string) $row['config_name'])) {
                $confirmBox_settings['S_' . strtoupper((string) $row['config_name'])] = $row['config_value'];
            }
        }
     ...
     * Socialnet
     */
    public function load_friends($user_id = 0, $type = 'user_id')
    {
    ...
    public function purge_friends($user_id = 0)
    {
        global $user, $cache;
    ...
    public function reload_friends($user_id = 0)
    {
        global $user;
        $user_id = $user_id == 0 ? $user->data['user_id'] : $user_id;
    ...
    public function load_groups($user_id = 0)
    {
        global $db, $user, $cache;
    ...
    public function purge_groups($user_id = 0)
    {
        global $cache, $user;
    ...
    public function reload_groups($user_id = 0)
    {
        global $user;
        $user_id = $user_id == 0 ? $user->data['user_id'] : $user_id;
    ...
    {
        $cls_filter_core = function($var) {
            return preg_match('/^sn_core_/si', (string) $var);
        };
     ...
     */
    public function get_user_avatar_resized($avatar = '', $avatar_type = 0, $avatar_width = 0, $avatar_height = 0, $max_height = 0, $stretch = true, $alt = 'USER_AVATAR', $ignore_config = false)
    {
        ...
        if (empty($avatar) || !$avatar_type || (!$config['allow_avatar'] && !$ignore_config) || !method_exists($this, 'get_user_avatar')) {
            if ($max_height < 25) {
                $img_sized = 'no_avatar';
                $size = 22;
            } elseif (abs(150 - $max_height) < abs(50 - $max_height)) {
                $img_sized = 'no_avatar_150';
                $size = 150;
            } elseif (abs(30 - $max_height) < abs(50 - $max_height)) {
     ...
     * @return string Avatar image
     */
    public function get_user_avatar($avatar, $avatar_type, $avatar_width, $avatar_height, $alt, $ignore_config = false)
    {
        ...
        return '<img src="' . (str_replace(' ', '%20', (string) $avatar_img)) . '" width="' . $avatar_width . '" height="' . $avatar_height . '" alt="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" />';
    }
 
    public function get_user_avatar_link($avatar, $avatar_type, $avatar_width, $avatar_height, $alt = 'USER_AVATAR', $ignore_config = false)
    {
    ...
    public function get_friend($mode, $user_id, $cfg_module, $cache_friends = true)
    {
     ...
     */
    public function get_username_string($cfg_module, $mode, $user_id, $username, $username_colour = '', $guest_username = false, $custom_profile_url = false, $cache_friends = true)
    {
        global $cache, $user, $phpbb_root_path;
 
        if (isset($this->friends['colourNames'][$user_id][$mode]) && $username != $user->lang['GUEST'] && !strpos((string) $this->friends['colourNames'][$user_id][$mode], (string) $user->lang['GUEST'])) {
        ...
        if (preg_match('/^full.*?$/si', (string) $mode, $match)) {
            $mode = 'full';
        }
    ...
    public function absolutePath($content)
    {
        return preg_replace_callback('/(src|href|action)=([\'"])([^\'"]+)([\'"])/si', 'snFunctions_absolutePath', (string) $content);
    }
     ...
     * @return void
     */
    public function page_header($page_title = '', $display_online_list = true, $item_id = 0, $item = 'forum')
    {
     ...
     */
    public function page_footer($block = 'body', $print = false)
    {
        global $template;
        ...
        if (!$print) {
            return $this->absolutePath(trim((string) $content));
        } else {
            print $this->absolutePath(trim((string) $content));
            exit();
        }
    }
     ...
     * @return string Stranka vygenerovana ze sablon $block
     */
    public function get_page($block = 'body', $print = false)
    {
        $this->page_header();
    ...
    public function gender_lang($lang, $user_id)
    {
        global $user;
 
        if ($user_id == $user->data['user_id']) {
            $gender = $user->data['sex'];
        } elseif (isset($this->friends['sex'][$user_id])) {
     ...
     * @return void
     */
    public function record_entry($user_id, $target, $type, $additionals = array()) // sn_core_entry->add
    {
        global $db;
     ...
     * @return void
     */
    public function delete_entry() // sn_core_entry->del
    {
        global $db;
    ...
    public function trim_text_withsmilies($text, $uid, $max_length, $max_paragraphs = 0, $stops = array(' ', "\n"), $replacement = '…', $bitfield = '', $enable_bbcode = true)
    {
        global $phpbb_root_path, $config;
 
        preg_match_all('/<!-- s([^ ]*) -->(<img.* \/>)<!-- s(\1) -->/Si', (string) $text, $matches);
        $text = preg_replace('/<!-- s([^ ]*) -->(<img.* \/>)<!-- s\1 -->/Si', '\1', (string) $text);
     ...
     */
    public function trim_text($text, $uid, $max_length, $max_paragraphs = 0, $stops = array(' ', "\n"), $replacement = '…', $bitfield = '', $enable_bbcode = true)
    {
        $orig_text = $text;
        ...
        if ($enable_bbcode) {
            ...
            if (empty($custom_bbcodes)) {
                ...
                while ($row = $db->sql_fetchrow($result)) {
                    ...
                    if ($this->str_ends_with((string) $row['bbcode_tag'], '=')) {
                        $custom_bbcodes[$row['bbcode_id']] = array('[' . $row['bbcode_tag'], ':' . $uid . ']', str_replace('$uid', $uid, (string) $row['second_pass_match']));
                    }
                }
                $db->sql_freeresult($result);
            }
        ...
        if ($max_length && utf8_strlen($text) > $max_length) {
            ...
            $text = rtrim((string) utf8_substr($text, 0, $max_length));
        ...
        foreach ($unsafe_tags as $tag) {
            ...
            if (($start_pos = strrpos((string) $text, (string) $tag[0])) > strrpos((string) $text, (string) $tag[1])) {
                ...
                if (isset($tag[2]) && preg_match($tag[2], substr($orig_text, $start_pos), $match, PREG_OFFSET_CAPTURE) != 0 && $match[0][1] === 0) {
                    $text = rtrim(substr((string) $text, 0, $start_pos));
                }
            }
        }
    ...
    public function hook_template()
    {
        ...
        if (defined('IN_ADMIN')) {
            ...
            if ($module->p_class == 'acp' && $module->p_name == 'update' && $module->p_mode == 'version_check') {
                $template->files['body'] = preg_replace('/(acp_)([^.]*\.html)/si', '\1socialnet_\2', (string) $template->files['body']);
                $template->filename['body'] = preg_replace('/(acp_)([^.]*\.html)/si', '\1socialnet_\2', (string) $template->filename['body']);
                $this->_version_checker(['host' => 'update.phpbb3hacks.com', 'directory' => '/socialnet', 'filename' => 'sn_modules.xml']);
            }
