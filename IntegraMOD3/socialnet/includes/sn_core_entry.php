...
class sn_core_entry extends sn_core_entry_gets
{
     ...
     */
    public function get_entry_array($entry_id)
    {
        global $db, $user;
     ...
     * @return array pole plne dat se vstupy na AP
     */
    public function get($last_time, $limit = 15, $older = true, $user_id = 0)
    {
        global $db, $user;
     ...
     */
    public function add($user_id, $target, $type, $additionals = array())
    {
        global $db;
...
class sn_core_entry_gets
{
     ...
     */
    public function entry_status($entry_uid, $entry_target, $entry_time)
    {
        global $db, $template;
     ...
     * Load FMS entries
     */
    public function entry_friends($entry_uid, $entry_target)
    {
        global $db;
     ...
     */
    public function entry_profile($entry_uid, $entry_target, $entry_additionals = '')
    {
                    ...
                    foreach ($entry_addArray as $field => $value) {
                        ...
                        if (!empty($entry_add)) {
                            if ($idx < $entry_size) {
                                ...
                                $entry_add .= ' ' . strtolower((string) $user->lang['AND']) . ' ';
                            }
                        }

                        $entry_add .= '<strong>';
                        $entry_add .= str_replace(' ', '&nbsp;', $user->lang[$field] ?? $user->lang['SN_UP_' . $field] ?? $user->lang[$field_] ?? "{ $field }");
     ...
     */
    public function entry_relation($entry_type, $entry_uid, $entry_target, $entry_additionals = '')
    {
     ...
     * Load Emotes entries
     */
    public function entry_emote($entry_uid, $entry_target, $entry_additionals = '')
    {
