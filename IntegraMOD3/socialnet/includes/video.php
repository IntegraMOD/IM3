...
class EmbeVi
{
    ...
    private bool $globalMaxImportant = false;
    //setting for keeping ratio
    private bool $keepRatio = false;
    //set embevi width
    private int $embeviWidth = 0;
    //set embevi height
    private int $embeviHeight = 0;
    ...
    private bool $acceptShortUrl = false;
    //use function htmlspecialchars_decode for parseHtml method
    private bool $specialCharDecode = true;
    ...
    private array $shortUrlServices = array(
      '2su.de/',
      '2.gp/',
      '2.ly/',
      '2ze.us/',
     ...
     * )
     */
    private array $embeviSupport = array(
      array(
     ...
     */
    public function setObjectAttr(mixed $param, $value = '')
    {
        if (!is_array($this->objectAttr)) {
            return false;
        }
     ...
     */
    public function setEmbedAttr(mixed $param, $value = '')
    {
     ...
     */
    public function setObjectParam(mixed $param, $value = '')
    {
        if (!is_array($this->objectParam)) {
            return false;
        }
            ...
            if(preg_match('/'.$v['matchExpr'].'/i', $url, $result)) {
                ...
                if(isset($v['flashvars'])) {
                    ...
                    for($regCount = 1; $regCount < count($result); $regCount++) {
                        $this->embedAttr['src'] = str_ireplace("~to_replace".$regCount."~", $result[$regCount], (string) $this->embedAttr['src']);
                        $this->embedAttr['flashvars'] = str_ireplace("~to_replace".$regCount."~", $result[$regCount], (string) $this->embedAttr['flashvars']);
                    }
        ...
        if(preg_match_all('/(https?[\S]+)/i', $text, $result)) {
            foreach($result[1] as $link) {
                if($this->parseUrl($link)) {
                    ...
                    $text = preg_replace('/'.preg_quote((string) $link, '/').'/i', $linkReplacement, $text);
                }
            }
        }
        return $text;
        ...
        if(preg_match_all('@(?<atag><a[^>]*?href ?= ?(?:"|\')(?<href>https?(?:.|\n)*?)(?:"|\').*?>(?<text>.*?)</a(?:[\s]+)?>)@i', $html, $result)) {
            ...
            for($i = 0; $i < count($result['atag']); $i++) {
                ...
                if($this->parseUrl($link)) {
                    ...
                    $html = preg_replace('/'.preg_quote((string) $result['atag'][$i], '/').'/i', $linkReplacement, $html);
                }
            }
    ...
    private function returnShortUrlServicesRegExpr()
    {
        ...
        foreach($shortUrlServices as $key => $value) {
            $shortUrlServices[$key] = preg_quote((string) $value, '/');
        }
