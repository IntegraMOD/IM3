...
if (!class_exists('socialnet_notify')) {
    ...
    class socialnet_notify
    {
        ...
        public function load()
        {
            ...
            if ($ntf_type == 'check') {
                ...
                for ($i = 0; isset($rowset[$i]); $i++) {
                    ...
                    if (!empty($ntf['link'])) {
                        $ntf_link = explode('?', (string) $ntf['link']);
                ...
                die(json_encode($ntf_return, JSON_THROW_ON_ERROR));
            }
        }
         ...
         * @return void
         */
        public function ntf_check_FAMILY($relation_id, $relative_user_id, $status_id)
        {
         ...
         * @return void
         */
        public function ntf_check_RELATIONSHIP($relation_id, $relative_user_id)
        {
            global $db, $user, $phpbb_root_path, $phpEx, $config;
            ...
            if ($this->p_master->script_name == 'activitypage') {
                ...
                if ($mode == 'notify') {
                    ...
                    for ($i = 0; isset($rowset[$i]); $i++) {
                        ...
                        $ntf_link = explode('?', (string) $data['link']);
         ...
         */
        public function ntf_mark($status, $from_status = SN_NTF_STATUS_NEW, $user = 0, $for = false)
        {
            global $db;
         ...
         * @param integer $user ID user, which belongs the notifications, 0 all users
         */
        public function ntf_markID($status, $from_status = SN_NTF_STATUS_NEW, $ntf = 0)
        {
            global $db;
        ...
        public function ntf_check_MARK()
        {
            global $db, $user;
         ...
         */
        public function ntf_delete($ntf_id = 0)
        {
            global $db, $user;
         ...
         */
        public function ntf_notify_count($status = SN_NTF_STATUS_UNREAD)
        {
            global $user, $db;
            $sql_where = array(
                "ntf_user = {$user->data['user_id']}",
