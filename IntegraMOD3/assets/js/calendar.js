...
function time_show(obj)
{
    ...
    let raw = obj.value.trim();
    let parts = raw.match(/(\d{1,2}):(\d{2})\s*(am|pm)/i);
    ...
    if (!parts) {
        ...
        current_hour = parts[1].padStart(2, '0');
        current_mins = parts[2];
        current_ampm = parts[3].toLowerCase();
        obj.value = current_hour + ':' + current_mins + ' ' + current_ampm;
    }

    document.getElementById('hrs').value = current_hour;
    document.getElementById('mins').value = current_mins;
    document.getElementById('ampm').value = current_ampm;
