...
if (!$auth->acl_gets('u_viewprofile', 'a_user', 'a_useradd', 'a_userdel')) {
    ...
    login_box('', ($user->lang['LOGIN_EXPLAIN_' . strtoupper('viewprofile')] ?? $user->lang['LOGIN_EXPLAIN_MEMBERLIST']));
}
...
if(!is_null($foe) && is_array($foe) && isset($row['foe']))
//$foe = ($row['foe']) ? true : false;
if(!is_null($friend) && is_array($friend) && isset($row['friend']))
...
if ($config['load_onlinetrack']) {
    ...
    $member['session_time'] = $session['session_time'] ?? 0;
    $member['session_viewonline'] = $session['session_viewonline'] ?? 0;
    unset($session);
...
$redirect = '&amp;redirect=' . base64_encode((string) append_sid("{$phpbb_root_path}profile.$phpEx", 'u=' . $user_id));
$template->assign_vars(array(
    ...
    'U_ADD_FRIEND'			 => (!$friend && !$request && !$foe && $friends_enabled) ? append_sid("{$phpbb_root_path}ucp.{$phpEx}", 'i=zebra&amp;add=' . urlencode(htmlspecialchars_decode((string) $member['username'])) . $redirect) : '',
    'U_ADD_FOE'				 => (!$friend && !$request && !$foe && $foes_enabled) ? append_sid("{$phpbb_root_path}ucp.{$phpEx}", 'i=zebra&amp;mode=foes&amp;add=' . urlencode(htmlspecialchars_decode((string) $member['username'])) . $redirect) : '',
...
while ($relation = $db->sql_fetchrow($result)) {
    ...
    if ($relation['family']) {
        ...
        if ($relation['anniversary']) {
            $relationship_arr = array_map('intval', explode('-', (string) $relation['anniversary']));
